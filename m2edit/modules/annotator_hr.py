...
class Annotator:
    ...
    def __call__(self,
        ...
        annotations_out = {}
        annotations_out["year"] = 2016
        annotations_out["text"] = "".join(src_tokens)
        annotations_out["correction"] = "".join(tgt_tokens)
        annotations_out["wrong"] = []
            ...
            for align_obj in align_objs:
                ...
                if edits not in edit_objs:
                    ...
                    for cor in cors:
                        ...
                        span.append(str(inds[0]))
                        span.append(str(inds[1]))
                        span.append(str(op))
                        annotations_out["wrong"].append(span)
                        # a_str = f"A {inds[0]} {inds[1]}|||{op}|||{toks}|||REQUIRED|||-NONE-|||{annotator_id}\n"
                        # annotations_out.append(a_str)
